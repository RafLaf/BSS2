...
def convertcurrent(ib,Cb=1.0,Chw=0.00038):
    '''
    yields the LSB input current (8LSB =1nAhw)
...
for i in range(n):
    ...
    for j in range(n):
        ...
        membrane_capacitance_capacitance=20,
        # Refractory time, range: 0-255
        refractory_period_refractory_time=10,   #changed as compared to LL4.npy but no noted elsewhere
...
np.save('data/LL6.npy',np.array(LL))
plt.show()
