...
def refereceVinv(refV):
    return (refV-0.5)*20/0.3-70

def MADCinv(MADC):
    return (MADC-700)/400*0.6+1


def plot_membrane_dynamics(population: Population,i ,segment_id=-1):
    """
    ...
    Vm=MADCinv(membrane)
    Vm=refereceVinv(Vm)
    plt.plot(times, Vm, alpha=0.5,label='current'+str(i))
...
for i in listi:
    ...
    stimulated_p.set(constant_current_enable=True, constant_current_i_offset=i)
    ...
    # be creative!
    pynn.run(0.1)
    plot_membrane_dynamics(stimulated_p,i=i)
    pynn.end()
plt.legend()
plt.savefig('v3.pdf')
plt.show()
